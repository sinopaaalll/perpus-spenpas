...
class Pengembalian_model extends CI_Model
{
    ...
    private function _get_datatables_query($search_value = null, $order_column = null, $order_dir = 'desc', $tgl_kembali = null, $status = null)
    {
        // Select kolom yang dibutuhkan
        $this->db->distinct();
        $this->db->select('peminjaman.kode, peminjaman.tgl_kembali, peminjaman.status, anggota.name as anggota');
        ...
        // Filter berdasarkan tanggal kembali jika tersedia
        if ($tgl_kembali) {
            $this->db->where('peminjaman.tgl_kembali', $tgl_kembali);
        }

        // Filter berdasarkan status jika tersedia
        if ($status == 'ALL') {
            $this->db->where_in('peminjaman.status', [2, 3]);
        } else {
    ...
    function get_datatables($limit = -1, $start = 0, $search_value = null, $order_column = null, $order_dir = 'desc', $tgl_kembali = null, $status = null)
    {
        // Set up query untuk datatables
        $this->_get_datatables_query($search_value, $order_column, $order_dir, $tgl_kembali, $status);
    ...
    function count_filtered($search_value = null, $tgl_kembali = null, $status = null)
    {
        $this->_get_datatables_query($search_value, null, 'desc', $tgl_kembali, $status);
        $query = $this->db->get();
